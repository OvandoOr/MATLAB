...
mwavelet='db7';                           % Wavelet basis function
...
b = size(y_c);
     ...
     hold on;
     sano = plot (y_c(1:round(b(2)/32)),'Color','black')
     grid on;
     ...
     axis([0 round(b(2)/32) -5 5]);
     
     
     subplot (4,1,2)
     hold on;
     plot (y_c(round(b(2)/32):round(b(2)/16)),'Color','black')
     grid on;
     ...
     subplot (4,1,3)
     hold on;
     plot (y_c(round(b(2)/16):round(b(2)/8)),'Color','black')
     grid on;

     yl=ylabel ('Det 4');
     set(yl,'fontsize',11,'fontweight','bold');
     axis([0 round(b(2)/16) -5 5]);
